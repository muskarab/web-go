...
func (h *userHandler) CheckEmailAvailability(c *gin.Context) {
	...
	if err != nil {
		errors := helper.FormatValidationError(err)
		errorMessage := gin.H{"errors": errors}
		response := helper.APIRespon("Email checking failed", http.StatusUnprocessableEntity, "error", errorMessage)
		c.JSON(http.StatusUnprocessableEntity, response)
		return
	}
	...
	response := helper.APIRespon(metaMessage, http.StatusOK, "success", data)
	c.JSON(http.StatusOK, response)
}
...
func (h *userHandler) UploadAvatar(c *gin.Context) {
	...
	if err != nil {
		...
		response := helper.APIRespon("Failed to upload avatar image", http.StatusBadRequest, "error", data)
		c.JSON(http.StatusBadRequest, response)
		return
	}
	...
	response := helper.APIRespon("Avatar successfuly uploaded", http.StatusOK, "success", data)
	c.JSON(http.StatusOK, response)
	return
}
